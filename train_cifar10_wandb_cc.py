...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train the cascade with optional Weights & Biases logging.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        ms_tracker = _start_codecarbon_tracker(
            ...
            f"{args.codecarbon_project_name}-ms",
        )
        ...
        print("Saved → model_s_pretrained.pth")
        if wandb_run is not None:
            wandb_run.save("model_s_pretrained.pth")
        ...
        ml_tracker = _start_codecarbon_tracker(
            codecarbon_enabled,
            args.codecarbon_output_dir,
            f"{args.codecarbon_project_name}-ml",
        )
        ...
        print("Saved → model_l_pretrained.pth")
        if wandb_run is not None:
            wandb_run.save("model_l_pretrained.pth")
        ...
        for alpha in _parse_alpha_values(args.alpha_values):
            ...
            gk_tracker = _start_codecarbon_tracker(
                codecarbon_enabled,
                args.codecarbon_output_dir,
                f"{args.codecarbon_project_name}-gk-alpha{alpha}",
            )
            ...
            checkpoint_name = f"model_s_gk_alpha{alpha}.pth"
