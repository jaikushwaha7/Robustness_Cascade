...
Evaluation Cifar10
============================
Drop-in replacement for the original eval script.

Adds the following previously-missing metrics to both the pkl results
and WandB logging:
...
def evaluate_cascade_stage(model_s_pretrained, model_l, val_loader,
    ...
    # ── Original metrics ─────────────────────────────────────────────────────
    correct_confs, incorrect_confs = collect_confidence_scores(model_s, val_loader, device)
    s_o, dens_c, dens_i, grid      = compute_distributional_overlap(correct_confs, incorrect_confs)
    (deferral_ratios, accs_real, accs_rand, accs_ideal,
     s_d, acc_s, acc_l)            = compute_deferral_performance(model_s, model_l, val_loader, device=device)
...
if __name__ == "__main__":
    ...
    # ── Baseline evaluation ───────────────────────────────────────────────────
    # Run first so baseline_acc_s and baseline_gain_pct are available for deltas
    (ms_stage_name, ms_results), ms_metrics = _run_tracked_stage(
        "MS", codecarbon_enabled,
        args.codecarbon_output_dir, args.codecarbon_project_name, wandb_run,
        lambda: evaluate_cascade_stage(
    ...
    for alpha in alphas:
        ...
        (alpha_stage_name, alpha_results), alpha_metrics = _run_tracked_stage(
            stage_label, codecarbon_enabled,
            args.codecarbon_output_dir, args.codecarbon_project_name, wandb_run,
            lambda alpha=alpha: evaluate_cascade_stage(
    ...
    if wandb_run is not None:
        ...
        # CodeCarbon stage table (unchanged)
        stage_order = ["GK_alpha_0.1","GK_alpha_0.3","GK_alpha_0.5",
                       "GK_alpha_0.7","GK_alpha_0.9","ML","MS"]
        stage_rows  = [stage_metrics_by_name[n] for n in stage_order
                       if n in stage_metrics_by_name]
        if stage_rows:
